  ...
  <ul :style="{ '--icon-size': iconSize }" class="navigation-tree">
    ...
    >
      ...
      >
        ...
        <UiIcon :icon="ChevronUpIcon" :size="arrowSize" class="arrow" />
      </button>
...
export type NavigationTreeLeaf = {
  title: string
  icon?: Component
  hint?: string
  disabled?: boolean
...
export type NavigationTreeBranch = {
  title: string
  icon?: Component
  hint?: string
  disabled?: boolean
...
export type NavigationTreeItem = NavigationTreeLeaf | NavigationTreeBranch

export type NavigationTreeProperties = {
  items: NavigationTreeItem[]
  iconSize?: string
  arrowSize?: string
}

export const isNavigationTreeBranch = (item: NavigationTreeItem): item is NavigationTreeBranch => {
...
const { items, iconSize = '1.5rem', arrowSize = '1rem' } = defineProps<NavigationTreeProperties>()
const emit = defineEmits<{
  navigate: [item: NavigationTreeLeaf]
}>()
...
const isLeafActive = (item: NavigationTreeLeaf): boolean => {
...
const isItemActive = (item: NavigationTreeItem): boolean => {
...
const onLeafClick = (event: MouseEvent, item: NavigationTreeLeaf, navigate: (event?: MouseEvent) => unknown): void => {
  if (item.disabled) {
