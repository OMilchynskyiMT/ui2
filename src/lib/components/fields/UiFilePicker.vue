  ...
  >
    ...
    >
      <template v-if="variant === 'field'" #leading>
        <slot name="icon">
          <UiIcon :icon="FileUpIcon" class="upload-icon" />
        </slot>
      </template>
        ...
        >
          <slot :accept="accept" :browse="browse" :files="model" :multiple="multiple" name="empty">
            <span class="area-icon">
              <slot name="icon">
                <UiIcon :icon="FileUpIcon" class="upload-icon" size="2.5rem" />
              </slot>
            </span>
...
export {
  ...
  type UiFilePickerVariant,
  type UiFileRejection,
  type UiFileRejectionReason,
} from './filePicker.shared'

export type UiFilePickerExpose = {
  browse: () => void
  clear: () => void
...
const emit = defineEmits<{
  change: [files: File[], source: UiFilePickerChangeSource, event?: Event]
  reject: [rejections: UiFileRejection[], source: 'picker' | 'drop', event: Event]
  cancel: [event: Event]
}>()
...
defineExpose<UiFilePickerExpose>({
  browse,
  clear,
