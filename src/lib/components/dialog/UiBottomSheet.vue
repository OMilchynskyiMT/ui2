  ...
  >
    <section class="sheet">
      ...
      <footer v-if="slots.actions" class="footer">
        <UiBottomActions :adaptive="actionsAdaptive" :sticky="false">
...
export type UiBottomSheetProperties = {
  title?: string
  description?: string
  headingId?: string
...
export type UiBottomSheetExposed = {
...
const {
  ...
  teleportTo = '#modals',
} = defineProps<UiBottomSheetProperties>()
...
const hasHeading = computed(() => Boolean(slots.title) || Boolean(title))
const hasHeader = computed(() => {
  return closeButton || Boolean(description) || hasHeading.value || Boolean(slots['header-actions'])
})
...
defineExpose<UiBottomSheetExposed>({ show, close, isVisible })
</script>
